...
const ACTIVE_SESSION_MESSAGE = 'You already have another booked session. Please complete that session first, then rate it before booking a new one.';
...
export default function ClientSessions() {
  ...
  const hasActiveBookedSession = () => {
    return mySessions.some(session => session.status !== 'Cancelled' && session.status !== 'Completed');
  };
  ...
  const startBooking = (pro) => {
    if (hasActiveBookedSession()) {
      setBookingError(ACTIVE_SESSION_MESSAGE);
      setBookingSuccess('');
      return;
    }
  ...
  const downloadReceiptAsPDF = async () => {
    ...
    try {
      const transactionDate = receiptModal.transactionDate || new Date();
      const receiptNumber = `KZN-${receiptModal.id}-${transactionDate.getFullYear()}`;
      const professionalName = receiptModal.professional?.professionalFullName || receiptModal.professional?.fullName || 'Professional';
      const doc = new jsPDF();
      const startY = addReceiptHeader(doc, 'Session Payment Receipt', [
        `Receipt #: ${receiptNumber}`,
        `Generated: ${new Date().toLocaleString()}`
      ]);
      const rows = [
        ['Receipt Number', receiptNumber],
        ['Transaction Date', transactionDate.toLocaleString('en-KE')],
        ['Professional', professionalName],
        ['Session Date & Time', receiptModal.formattedDate || new Date(receiptModal.sessionDate).toLocaleString('en-KE')],
        ['Payment Method', 'M-Pesa'],
        ['Payment Status', 'Paid'],
        ['Total Amount', 'KSh 1,500'],
        ['Generated By', getReportGeneratorName()]
      ];
      const finalY = addReceiptDetailsTable(doc, 'Payment Details', rows, startY);
      addReceiptFooter(doc, finalY, 'This receipt serves as proof of payment.');
      doc.save(`receipt_${receiptModal.id}.pdf`);
    } catch (error) {
  ...
  return (
    <div>
      ...
      {bookingError && !selectedPro && (
        <div style={{ background: PINK_LIGHT, color: PINK, padding: '12px 16px', borderRadius: 10, marginBottom: 20, fontSize: 14, border: `1px solid ${PINK}` }}>
          {bookingError}
          <button onClick={() => setBookingError('')} style={{ float: 'right', background: 'none', border: 'none', cursor: 'pointer', color: PINK, fontSize: 16, width: 'auto' }}>×</button>
        </div>
      )}
      {paySuccess && (
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 18, maxWidth: 460, width: '100%', border: `1.5px solid ${PINK}`, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            <div id="receipt-content" style={{ padding: 32, overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{ background: `linear-gradient(135deg, ${PINK_LIGHT}, ${PURPLE_LIGHT})`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ marginBottom: 12 }}><p style={{ fontSize: 11, color: 'var(--text-muted)', margin: 0 }}>Generated By</p><p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', margin: '4px 0 0' }}>{getReportGeneratorName()}</p></div>
