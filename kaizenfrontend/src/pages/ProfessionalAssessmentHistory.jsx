// src/pages/ProfessionalAssessmentHistory.jsx
...
export default function ProfessionalAssessmentHistory() {
  ...
  const fetchClientAssessments = async () => {
    setLoading(true);
    try {
      const response = await API.get(`/professional/client-assessments/${clientId}`);
      setClient(response.data.client);
      setAssessments(response.data.assessments);
    } catch (err) {
      console.error('Failed to fetch client assessments:', err);
      setError('Could not load client assessment history.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getScoreTrend = (current, previous) => {
    ...
    if (current < previous) return { text: 'Improved', color: 'var(--success-text)', icon: '📉' };
    if (current > previous) return { text: 'Declined', color: 'var(--error-text)', icon: '📈' };
    return { text: 'Stable', color: 'var(--text-muted)', icon: '➡️' };
  };
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px' }}>
        <div style={{
          ...
          <p style={{ color: 'var(--text-muted)', marginBottom: 24 }}>
            {client?.firstName} {client?.lastName} hasn't completed any assessments yet.
          </p>
          <button 
            onClick={() => navigate('/professional-sessions')}
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        <div style={{
          ...
          <p style={{ fontSize: 32, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{assessments.length}</p>
        </div>
        <div style={{
          background: 'var(--bg-card)',
          borderRadius: 16,
          ...
          <p style={{ fontSize: 32, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{assessments[0]?.overallScore}/5</p>
        </div>
        <div style={{
          background: 'var(--bg-card)',
          borderRadius: 16,
          ...
          <p style={{ fontSize: 32, fontWeight: 700, color: 'var(--success-text)', margin: 0 }}>
            {(assessments.reduce((sum, a) => sum + a.overallScore, 0) / assessments.length).toFixed(1)}/5
          </p>
        </div>
      </div>
      ...
      <div style={{
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {assessments.map((assessment, index) => {
                ...
                const trend = getScoreTrend(assessment.overallScore, previousAssessment?.overallScore);
        ...
        {assessments.length >= 2 && (
          <div style={{ 
            marginTop: 24, 
            padding: 16, 
            background: assessments[0].overallScore < assessments[1].overallScore ? 'var(--success-bg)' : 
                        assessments[0].overallScore > assessments[1].overallScore ? 'var(--error-bg)' : 'var(--bg-secondary)',
            borderRadius: 12
          }}>
            <p style={{ 
              fontSize: 13, 
              color: assessments[0].overallScore < assessments[1].overallScore ? 'var(--success-text)' : 
                     assessments[0].overallScore > assessments[1].overallScore ? 'var(--error-text)' : 'var(--text-muted)',
              margin: 0,
              display: 'flex',
              alignItems: 'center',
              gap: 8
            }}>
              {assessments[0].overallScore < assessments[1].overallScore ? '🎉' : 
               assessments[0].overallScore > assessments[1].overallScore ? '📉' : '➡️'}
              <strong>Progress Summary:</strong> {client?.firstName}'s overall wellbeing has {
                assessments[0].overallScore < assessments[1].overallScore ? 'improved' :
                assessments[0].overallScore > assessments[1].overallScore ? 'declined' : 'remained the same'
              } since their last assessment.
              {assessments[0].overallScore < assessments[1].overallScore && ' This is positive progress! 🌟'}
              {assessments[0].overallScore > assessments[1].overallScore && ' Consider checking in with them to offer additional support. 💪'}
            </p>
          </div>
        )}
        ...
        <div style={{ 
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16, lineHeight: 1.6 }}>
            Based on the assessment history, {client?.firstName} has shown {
              assessments.length > 1 && assessments[0].overallScore < assessments[1].overallScore ? 'improvement' :
              assessments.length > 1 && assessments[0].overallScore > assessments[1].overallScore ? 'some challenges' : 'consistent'
            } in their mental wellbeing over time.
          </p>
