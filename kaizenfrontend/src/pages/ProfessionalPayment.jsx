...
import { addReceiptDetailsTable, addReceiptFooter, addReceiptHeader, getReportGeneratorName } from '../utils/pdfReportBranding';
...
export default function ProfessionalPayment() {
  ...
  const downloadReceiptAsPDF = async () => {
    ...
    try {
      ...
      const transactionDate = receiptModal.date ? new Date(receiptModal.date) : generatedAt;
      const doc = new jsPDF();
      const receiptNumber = `PAY-${receiptModal.id}-${Date.now()}`;
      ...
      const rows = [
        ...
        ['Payment Method', receiptModal.method || 'Manual Transfer'],
        ['Payment Account', currentMethod?.detail || receiptModal.paymentAccount || 'N/A'],
        ['Payment Status', receiptModal.status || 'Completed'],
        ['Total Amount', formatCurrency(receiptModal.amount)],
        ['Generated By', getReportGeneratorName()]
      ];
      const finalY = addReceiptDetailsTable(doc, 'Payout Details', rows, startY);
      addReceiptFooter(doc, finalY, 'This is an official payout transaction receipt.');
      doc.save(`payout_receipt_${receiptModal.id}.pdf`);
    } catch (error) {
