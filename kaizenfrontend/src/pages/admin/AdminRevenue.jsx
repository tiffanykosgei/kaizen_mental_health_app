...
import { addReceiptDetailsTable, addReceiptFooter, addReceiptHeader, addReportHeader, getReportGeneratorName } from '../../utils/pdfReportBranding';
...
export default function AdminRevenue() {
  ...
  const fetchRevenueData = async () => {
    ...
    try {
      ...
      const updatedProfessionals = (response.data.professionals || []).map(pro => ({
        ...pro,
        totalEarned: (pro.totalEarned || 0) * CONVERSION_FACTOR,
        platformFees: (pro.platformFees || 0) * CONVERSION_FACTOR,
        pendingPayout: (pro.pendingPayout || 0) * CONVERSION_FACTOR,
        paidOut: (pro.paidOut || 0) * CONVERSION_FACTOR
      }));
      setSummary({
        ...response.data.summary,
        totalPlatformFees: (response.data.summary?.totalPlatformFees || 0) * CONVERSION_FACTOR,
        totalProfessionalEarnings: (response.data.summary?.totalProfessionalEarnings || 0) * CONVERSION_FACTOR,
        totalRevenue: (response.data.summary?.totalRevenue || 0) * CONVERSION_FACTOR
      });
  ...
  const downloadReceiptAsPDF = async () => {
    ...
    try {
      ...
      const doc = new jsPDF();
      const receiptNumber = `PAY-${receiptModal.professionalId}-${Date.now()}`;
      const startY = addReceiptHeader(doc, 'Professional Payout Receipt', [
        `Receipt #: ${receiptNumber}`,
        `Generated: ${generatedAt.toLocaleString()}`
      ]);
      const rows = [
        ['Professional', receiptModal.professionalName],
        ['Email', receiptModal.professionalEmail],
        ['Payment Method', receiptModal.paymentMethod || 'Manual Transfer'],
        ['Payment Account', receiptModal.paymentAccount || 'N/A'],
        ['Total Amount Paid', `KSh ${(receiptModal.paidOut || 0).toLocaleString()}`],
        ['Total Sessions', String(receiptModal.totalSessions || 0)],
        ['Payment Date', generatedAt.toLocaleString()],
        ['Status', 'Completed'],
        ['Generated By', getReportGeneratorName()]
      ];
      const finalY = addReceiptDetailsTable(doc, 'Payout Details', rows, startY);
      addReceiptFooter(doc, finalY, 'This is a system-generated payout receipt.');
      doc.save(`payout_receipt_${receiptModal.professionalName.replace(/\s/g, '_')}.pdf`);
    } catch (error) {
  ...
  const exportToPDF = () => {
    try {
      ...
      const startY = addReportHeader(doc, reportTitle, [
        ...
        `Total Revenue: ${formatCurrency(filteredSummary.totalRevenue)}`,
  ...
  const filteredSummary = {
    ...
    totalRevenue: filteredProfessionals.reduce((sum, p) => sum + (p.platformFees || 0) + (p.totalEarned || 0), 0)
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
        ...
        <StatsCard title="Total Revenue" value={formatCurrency(filteredSummary.totalRevenue)} color="purple" icon="💵" />
      </div>
              ...
              <tbody>
                {filteredProfessionals.map(pro => (
                  <tr key={pro.professionalId} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 16px', textAlign: 'right', fontSize: 13, color: '#e91e8c' }}>{formatCurrency(pro.platformFees)}</td>
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 18, maxWidth: 460, width: '100%', border: `1.5px solid ${PINK}`, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            <div id="receipt-content" style={{ padding: 32, overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{ background: `linear-gradient(135deg, ${PINK_LIGHT}, ${PURPLE_LIGHT})`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: 0 }}>Generated By</p>
                  <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', margin: '4px 0 0' }}>{getReportGeneratorName()}</p>
                </div>
