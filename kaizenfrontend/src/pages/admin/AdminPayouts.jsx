...
export default function AdminPayouts() {
  ...
  const downloadReceiptAsPDF = async () => {
    ...
    try {
      generatePayoutReceipt(receiptModal, receiptModal.paidOut || 0, receiptModal.totalSessions || 0);
    } catch (error) {
  ...
  const generatePayoutReceipt = (professional, amount, sessions, payoutDate = new Date()) => {
    try {
      const doc = new jsPDF();
      const receiptNumber = `PAY-${professional.professionalId}-${Date.now()}`;
      const startY = addReceiptHeader(doc, 'Professional Payout Receipt', [
        `Receipt #: ${receiptNumber}`,
        `Generated: ${payoutDate.toLocaleString()}`
      ]);
      ...
      const finalY = addReceiptDetailsTable(doc, 'Payout Details', tableData, startY);
